<template>
	<view class="content">
		<view class="search-box">
			...
			<mSearch class="mSearch-input-box" :mode="2" button="inside" :placeholder="defaultKeyword" @search="doSearch(false)" @input="inputChange" @confirm="doSearch(false)" v-model="keyword.searchWord"></mSearch>
		</view>
		<view class="search-keyword" >
			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					...
					<view class="keyword">
						<view v-for="(keyword,index) in oldKeywordList" @tap="doSearch(keyword)" :key="index">{{keyword.searchWord}}</view>
					</view>
				</view>
				<view class="keyword-block">
					...
					<view class="keyword" v-if="forbid==''">
						<view v-for="(keyword,index) in hotKeywordList" @tap="doSearch(keyword)" :key="index">{{keyword.searchWord}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			inputChange(event) {
				...
				uni.request({
					url:this.$myRequest+'/cloudsearch?keywords=' + keyword,
